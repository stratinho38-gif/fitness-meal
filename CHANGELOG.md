...
## v8.7.0 — 2026-08-24 (Έξυπνη προσθήκη υλικών — aggregate ποσοτήτων)

- **Οι ποσότητες πλέον αθροίζονται αντί για διπλά entries**: αν η λίστα έχει «Κοτόπουλο 2 κιλά» και προσθέσεις συνταγή με «400γρ στήθος κοτόπουλο», το υπάρχον γίνεται **2,4 κιλά** (και ξετσεκάρεται αν ήταν ✓). Ίδιες μονάδες αθροίζονται (π.χ. κονσέρβες τόνου), γρ↔κιλά μετατρέπονται, μη μετατρέψιμες (φλ, κ.σ., φέτες) κάνουν **+1** στο υπάρχον· η σημείωση αποκτά «Για: <γεύμα>».
- **Λεξικό υλικών** (`ING_DICT`, ~110 προϊόντα): κάθε γραμμή συνταγής σπάει σε μέρη («Κρεμμύδι, σκόρδο, δάφνη» → 3 προϊόντα) και αναγνωρίζεται ως κανονικό προϊόν με σωστή κατηγορία — «400γρ κιμάς μοσχαρίσιος» μπαίνει ως «Κιμάς μοσχαρίσιος άπαχος, 400 γρ, Κρέας & Ψάρι» αντί για 1 τεμ στα «Άλλα». **Κάλυψη 100%** των υλικών και των 150 συνταγών (εγγυημένο με test).
- **Μικρο-υλικά** (λάδι, λεμόνι, ρίγανη, αλάτι/πιπέρι, μπαχαρικά κλπ) εμφανίζονται **ξετσεκαρισμένα** στο modal — τα τσεκάρεις μόνο αν σου τελειώνουν. Το «Ψώνια εβδομάδας» τα παραλείπει και περνάει κι αυτό από την ίδια έξυπνη μηχανή (aggregate σε όλη την εβδομάδα).
- Νέες pure functions `ingSplit`/`ingMatch`/`ingParseQty`/`mergeQty`/`ingPlan`/`weekPlan` (αντικαθιστούν τις `mealIngItems`/`weekIngItems`) + εφαρμογή πλάνου με ένα Firebase update (`applyIngPlan`). 173/173 tests πράσινα.
