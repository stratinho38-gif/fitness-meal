...
Έκδοση: **v8.7 (Έξυπνη προσθήκη υλικών — aggregate ποσοτήτων)** · Αρχείο εφαρμογής: `index.html` · Deployed: Netlify `lista-supemarket` + Cowork artifact `lista-psonon-stratos`
...
### Υλικά γεύματος → λίστα ψώνων (v8.2 · επιλογή v8.4 · έξυπνο aggregate v8.7)

- Στο modal συνταγής, τα «Υλικά» εμφανίζονται με **checkbox ανά υλικό** (`.ing-chk` με `data-i` = index στο `r.ing`). **Μικρο-υλικά** (γραμμές όπου όλα τα μέρη είναι `m:true` στο λεξικό — καρυκεύματα, λάδι, λεμόνι, αλάτι/πιπέρι) είναι **ξετσεκαρισμένα by default** (`ingLineMinor`). Από κάτω: επιλογέας λίστας (default η ενεργή) + κουμπί **«Στη λίστα (N)»** (live μετρητής). Προστίθενται **μόνο τα τσεκαρισμένα**· κανένα τσεκαρισμένο → toast, κανένα write.
- **Έξυπνη μηχανή (v8.7)** — pure functions πριν το Room block, όλες testable:
  - `ingSplit(line)`: σπάει γραμμή σε μέρη σε κόμμα/«+» εκτός παρενθέσεων (το δεκαδικό κόμμα «1,5» δεν κόβει).
  - `ING_DICT` (~110 entries, **η σειρά μετράει** — ειδικά πριν από γενικά, π.χ. κιμάς πριν από κοτόπουλο, γλυκοπατάτα πριν από πατάτα, πιπεριά πριν από πιπέρι): στελέχη λέξεων → κανονικό όνομα προϊόντος + κατηγορία + flag `m` (μικρο-υλικό). **Κάλυψη 100%** των μερών όλων των 150 συνταγών (test το εγγυάται).
  - `ingMatch(text)`: κανονικοποίηση (`ingNorm`: πεζά/χωρίς τόνους/ς→σ, `ingStripParens`) και ταίριασμα με το λεξικό (στέλεχος με κενό = substring, αλλιώς αρχή λέξης).
  - `ingParseQty(text)`: «400γρ …» → γρ, «1,5 κιλό» → κιλά, «1,5 κονσέρβα» → κονσέρβες, «2 πατάτες»/«1/2 …» → τεμ· μη μετατρέψιμες μονάδες (φλ, κ.σ., κ.γλ., φέτες, σκουπ, κλωνάρια, σκελίδες, πρέζες) → `null`.
  - `mergeQty(exQty, exUnit, addQty, addUnit)`: γρ↔κιλά αθροίζονται με μετατροπή (2 κιλά + 400γρ = 2,4 κιλά), ίδια μονάδα αθροίζεται, `null`/ασύμβατη → **+1** στη μονάδα του υπάρχοντος.
  - `ingPlan(lines, existingItems, mealName)` / `ingPlanPairs(pairs, existing)`: ομαδοποιεί τα μέρη ανά προϊόν (aggregate και μέσα στη συνταγή), ταιριάζει με υπάρχοντα items **μέσω λεξικού** (π.χ. «400γρ στήθος κοτόπουλο» βρίσκει το «Κοτόπουλο (στήθος/μπούτι φιλέτο)» της λίστας) ή με ίδιο κανονικοποιημένο όνομα → `{ adds, updates }`. Updates: νέα ποσότητα, `done:false` (ξαναγίνεται για αγορά), note με προσθήκη «Για: <γεύμα>» αν λείπει (όριο 120). Adds: κανονικό όνομα/κατηγορία από λεξικό, πραγματική ποσότητα/μονάδα (γρ ≥1000 → κιλά), note «Για: <γεύματα>»· fallback χωρίς ταίριασμα: όνομα το κείμενο του μέρους, 1 τεμ, «Άλλα».
- Η εφαρμογή του πλάνου γίνεται με **ένα** Firebase update (`applyIngPlan`: adds ως νέα keys + updates ως subpaths `id/qty`, `id/done`, `id/note`) — ένα `.catch(dbErr)`. Toast «Λίστα «Χ»: N νέα · M ενημερώθηκαν».
- Αν δεν υπάρχει καμία λίστα → `newListFromPlan`: νέα λίστα **«Ψώνια»** με τα adds, γίνεται ενεργή.
- Ονόματα λιστών στον επιλογέα μπαίνουν μόνο με `textContent` (user input). Τα δεδομένα συνταγών/λεξικού είναι στατικά.
...
- **«Ψώνια εβδομάδας»** (κουμπί στο toolbar των Γευμάτων): μαζεύει τα υλικά και των **14 γευμάτων** της εβδομάδας στην ενεργή λίστα, με confirm πρώτα. **v8.7**: περνά από την ίδια έξυπνη μηχανή (`weekPlan` → `ingPlanPairs`): aggregate ποσοτήτων μέσα στην εβδομάδα και με τα υπάρχοντα items, **τα μικρο-υλικά παραλείπονται** (όπως το default του modal), note «Για: <γεύματα>». Αν δεν υπάρχει λίστα → νέα «Ψώνια» (ενεργή). Τίποτα νέο → toast, κανένα write.
